...
public class HashPartition<BT, PT> extends AbstractPagedInputView implements SeekableDataInputView {
    ...
    HashPartition(
        ...
        this.partitionBuffers =
                (MemorySegment[]) buffers.toArray(new MemorySegment[buffers.size()]);
    ...
    public void finalizeBuildPhase(
        ...
        if (!isInMemory()) {
            ...
            this.probeSideChannel =
                    ioAccess.createBlockChannelWriter(
                            probeChannelEnumerator.next(), bufferReturnQueue);
            // creating the ChannelWriterOutputView without memory will cause it to draw one segment
            // from the
            // write behind queue, which is the spare segment we had above.
            this.probeSideBuffer =
                    new ChannelWriterOutputView(this.probeSideChannel, this.memorySegmentSize);
        }
    }
    ...
    final SeekableDataOutputView getWriteView() {
        if (this.overwriteBuffer == null) {
            this.overwriteBuffer =
                    new RandomAccessOutputView(this.partitionBuffers, this.memorySegmentSize);
        }
    ...
    public void prepareProbePhase(
        ...
        this.probeSideChannel =
                ioAccess.createBlockChannelWriter(probeChannelEnumerator.next(), bufferReturnQueue);
        this.probeSideBuffer =
                new ChannelWriterOutputView(this.probeSideChannel, this.memorySegmentSize);
    }
    ...
    public void setReadPosition(long pointer) {
        ...
        seekInput(
                ...
                offset,
                bufferNum < this.partitionBuffers.length - 1
                        ? this.memorySegmentSize
