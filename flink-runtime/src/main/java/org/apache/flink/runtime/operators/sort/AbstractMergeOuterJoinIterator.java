...
public abstract class AbstractMergeOuterJoinIterator<T1, T2, O>
    ...
    private void joinLeftKeyValuesWithNull(
            Iterator<T1> values, FlatJoinFunction<T1, T2, O> joinFunction, Collector<O> collector)
            throws Exception {
    ...
    private void joinRightKeyValuesWithNull(
            Iterator<T2> values, FlatJoinFunction<T1, T2, O> joinFunction, Collector<O> collector)
            throws Exception {
        while (values.hasNext()) {
            T2 next = values.next();
