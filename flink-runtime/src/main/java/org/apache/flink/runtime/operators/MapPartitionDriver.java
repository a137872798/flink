...
/**
 * MapPartition task which is executed by a Task Manager. The task has a single input and one or
 * multiple outputs. It is provided with a MapFunction implementation.
 *
 * <p>The MapPartitionTask creates an iterator over all key-value pairs of its input and hands that
 * to the <code>map_partition()</code> method of the MapFunction.
 *
...
public class MapPartitionDriver<IT, OT> implements Driver<MapPartitionFunction<IT, OT>, OT> {
    ...
    public Class<MapPartitionFunction<IT, OT>> getStubType() {
        ...
        final Class<MapPartitionFunction<IT, OT>> clazz =
                (Class<MapPartitionFunction<IT, OT>>) (Class<?>) MapPartitionFunction.class;
        return clazz;
    }
    ...
    public void prepare() {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "MapPartitionDriver object reuse: "
    ...
    public void run() throws Exception {
        ...
        final MutableObjectIterator<IT> input =
                new CountingMutableObjectIterator<>(this.taskContext.<IT>getInput(0), numRecordsIn);
        ...
        if (objectReuseEnabled) {
            final ReusingMutableToRegularIteratorWrapper<IT> inIter =
                    new ReusingMutableToRegularIteratorWrapper<IT>(
                            input, this.taskContext.<IT>getInputSerializer(0).getSerializer());

            function.mapPartition(inIter, output);
        } else {
            final NonReusingMutableToRegularIteratorWrapper<IT> inIter =
                    new NonReusingMutableToRegularIteratorWrapper<IT>(
                            input, this.taskContext.<IT>getInputSerializer(0).getSerializer());
