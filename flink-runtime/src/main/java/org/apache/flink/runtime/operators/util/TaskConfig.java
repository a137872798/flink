...
public class TaskConfig implements Serializable {
    ...
    private static final String BROADCAST_INPUT_TYPE_SERIALIZER_FACTORY_PREFIX =
            "in.bc.serializer.";

    private static final String INPUT_TYPE_SERIALIZER_PARAMETERS_PREFIX = "in.serializer.param.";

    private static final String BROADCAST_INPUT_TYPE_SERIALIZER_PARAMETERS_PREFIX =
            "in.bc.serializer.param.";
    ...
    private static final String INPUT_STRATEGY_COMPARATOR_PARAMETERS_PREFIX =
            "in.comparator.param.";
    ...
    private static final String SORT_SPILLING_THRESHOLD_INPUT_PREFIX =
            "sort-spill-threshold.input.";
    ...
    private static final String NUMBER_OF_EOS_EVENTS_BROADCAST_PREFIX =
            "iterative.num-eos-events.bc.";
    ...
    private static final String ITERATION_HEAD_INDEX_OF_PARTIAL_SOLUTION =
            "iterative.head.ps-input-index";

    private static final String ITERATION_HEAD_INDEX_OF_SOLUTIONSET =
            "iterative.head.ss-input-index";

    private static final String ITERATION_HEAD_BACKCHANNEL_MEMORY =
            "iterative.head.backchannel-memory";

    private static final String ITERATION_HEAD_SOLUTION_SET_MEMORY =
            "iterative.head.solutionset-memory";
    ...
    private static final String ITERATION_IMPLICIT_CONVERGENCE_CRITERION_AGG_NAME =
    ...
    private static final String ITERATION_SOLUTION_SET_SERIALIZER_PARAMETERS =
            "iterative.ss-serializer.params";

    private static final String ITERATION_SOLUTION_SET_COMPARATOR = "iterative.ss-comparator";

    private static final String ITERATION_SOLUTION_SET_COMPARATOR_PARAMETERS =
            "iterative.ss-comparator.params";

    private static final String ITERATION_SOLUTION_SET_UPDATE = "iterative.ss-update";

    private static final String ITERATION_SOLUTION_SET_UPDATE_SKIP_REPROBE =
            "iterative.ss-update-fast";
    ...
    public <S extends Function, OT> Class<? extends Driver<S, OT>> getDriver() {
        ...
        try {
            ...
            throw new CorruptConfigurationException(
                    "The given driver class does not implement the pact driver interface.");
        }
    }
    ...
    public DriverStrategy getDriverStrategy() {
        ...
        if (ls == -1) {
            ...
            throw new CorruptConfigurationException(
                    "Illegal driver strategy in configuration: " + ls);
        } else {
    ...
    public void setDriverComparator(TypeComparatorFactory<?> factory, int inputNum) {
        setTypeComparatorFactory(
                factory,
                DRIVER_COMPARATOR_FACTORY_PREFIX + inputNum,
    ...
    public <T> TypeComparatorFactory<T> getDriverComparator(int inputNum, ClassLoader cl) {
        return getTypeComparatorFactory(
                DRIVER_COMPARATOR_FACTORY_PREFIX + inputNum,
                DRIVER_COMPARATOR_PARAMETERS_PREFIX + inputNum + SEPARATOR,
                cl);
    }
    ...
    public <T1, T2> TypePairComparatorFactory<T1, T2> getPairComparatorFactory(ClassLoader cl) {
        ...
        @SuppressWarnings("unchecked")
        final Class<TypePairComparatorFactory<T1, T2>> superClass =
                (Class<TypePairComparatorFactory<T1, T2>>)
                        (Class<?>) TypePairComparatorFactory.class;
        try {
            final Class<? extends TypePairComparatorFactory<T1, T2>> clazz =
            ...
            throw new CorruptConfigurationException(
                    "The class noted in the configuration as the pair comparator factory "
    ...
    public LocalStrategy getInputLocalStrategy(int inputNum) {
        ...
        if (ls == -1) {
            ...
            throw new CorruptConfigurationException(
                    "Illegal local strategy in configuration: " + ls);
        } else {
    ...
    public void setInputSerializer(TypeSerializerFactory<?> factory, int inputNum) {
        setTypeSerializerFactory(
                factory,
                INPUT_TYPE_SERIALIZER_FACTORY_PREFIX + inputNum,
    ...
    public void setBroadcastInputSerializer(TypeSerializerFactory<?> factory, int inputNum) {
        setTypeSerializerFactory(
                factory,
                BROADCAST_INPUT_TYPE_SERIALIZER_FACTORY_PREFIX + inputNum,
    ...
    public <T> TypeSerializerFactory<T> getInputSerializer(int inputNum, ClassLoader cl) {
        return getTypeSerializerFactory(
                INPUT_TYPE_SERIALIZER_FACTORY_PREFIX + inputNum,
                INPUT_TYPE_SERIALIZER_PARAMETERS_PREFIX + inputNum + SEPARATOR,
                cl);
    }
    ...
    public void setInputComparator(TypeComparatorFactory<?> factory, int inputNum) {
        setTypeComparatorFactory(
                factory,
                INPUT_STRATEGY_COMPARATOR_FACTORY_PREFIX + inputNum,
    ...
    public String getBroadcastInputName(int groupIndex) {
        return this.config.getString(
                BROADCAST_INPUT_NAME_PREFIX + groupIndex,
                String.format("broadcastVar%04d", groupIndex));
    }
    ...
    public void setOutputSerializer(TypeSerializerFactory<?> factory) {
        setTypeSerializerFactory(
                factory, OUTPUT_TYPE_SERIALIZER_FACTORY, OUTPUT_TYPE_SERIALIZER_PARAMETERS_PREFIX);
    }

    public <T> TypeSerializerFactory<T> getOutputSerializer(ClassLoader cl) {
        return getTypeSerializerFactory(
                OUTPUT_TYPE_SERIALIZER_FACTORY, OUTPUT_TYPE_SERIALIZER_PARAMETERS_PREFIX, cl);
    }

    public void setOutputComparator(TypeComparatorFactory<?> factory, int outputNum) {
        setTypeComparatorFactory(
                factory,
                OUTPUT_TYPE_COMPARATOR_FACTORY_PREFIX + outputNum,
    ...
    public void setOutputDataDistribution(DataDistribution distribution, int outputNum) {
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
            ...
            throw new RuntimeException(
                    "Error serializing the DataDistribution: " + e.getMessage(), e);
        }
    }
    ...
    public void setOutputPartitioner(Partitioner<?> partitioner, int outputNum) {
        try {
            InstantiationUtil.writeObjectToConfig(
                    partitioner, config, OUTPUT_PARTITIONER + outputNum);
        } catch (Throwable t) {
    ...
    public TaskConfig getChainedStubConfig(int chainPos) {
        return new TaskConfig(
                new DelegatingConfiguration(
                        this.config, CHAINING_TASKCONFIG_PREFIX + chainPos + SEPARATOR));
    }
    ...
    public Class<? extends ChainedDriver<?, ?>> getChainedTask(int chainPos) {
        ...
        @SuppressWarnings("unchecked")
        final Class<ChainedDriver<?, ?>> clazz =
                (Class<ChainedDriver<?, ?>>) (Class<?>) ChainedDriver.class;
        try {
    ...
    public void setGateIterativeWithNumberOfEventsUntilInterrupt(
            int inputGateIndex, int numEvents) {
        if (inputGateIndex < 0) {
            throw new IllegalArgumentException();
        }
        if (numEvents <= 0) {
    ...
    public TaskConfig getIterationHeadFinalOutputConfig() {
        return new TaskConfig(
                new DelegatingConfiguration(this.config, ITERATION_HEAD_FINAL_OUT_CONFIG_PREFIX));
    }
    ...
    public void setSolutionSetComparator(TypeComparatorFactory<?> factory) {
        setTypeComparatorFactory(
                factory,
                ITERATION_SOLUTION_SET_COMPARATOR,
    ...
    public void addIterationAggregator(String name, Aggregator<?> aggregator) {
        ...
        try {
            InstantiationUtil.writeObjectToConfig(
                    aggregator, this.config, ITERATION_AGGREGATOR_PREFIX + num);
        } catch (IOException e) {
            throw new RuntimeException(
    ...
    public void addIterationAggregators(Collection<AggregatorWithName<?>> aggregators) {
        ...
        for (AggregatorWithName<?> awn : aggregators) {
            ...
            try {
                InstantiationUtil.writeObjectToConfig(
                        awn.getAggregator(), this.config, ITERATION_AGGREGATOR_PREFIX + num);
            } catch (IOException e) {
                throw new RuntimeException(
                        "Error while writing the aggregator object to the task configuration.");
            }
    ...
    @SuppressWarnings("unchecked")
    public <T extends Value> ConvergenceCriterion<T> getImplicitConvergenceCriterion(
            ClassLoader cl) {
        ConvergenceCriterion<T> convCriterionObj;
        try {
            convCriterionObj =
            ...
            throw new RuntimeException(
                    "Error while reading the default convergence criterion object from the task configuration.");
        } catch (ClassNotFoundException e) {
            throw new RuntimeException(
                    "Error while reading the default convergence criterion object from the task configuration. "
                            + "ConvergenceCriterion class not found.");
    ...
    private void setTypeSerializerFactory(
    ...
    private <T> TypeSerializerFactory<T> getTypeSerializerFactory(
    ...
    private <T> TypeComparatorFactory<T> getTypeComparatorFactory(
            String classNameKey, String parametersPrefix, ClassLoader cl) {
        // check the class name
        final String className = this.config.getString(classNameKey, null);
        if (className == null) {
