 ...
 *
 * <p>The methods in this interface depend not only on the record, but also on what fields of a
 * record are used for the comparison or hashing. That set of fields is typically a subset of a
 * record's fields. In general, this class assumes a contract on hash codes and equality the same
 * way as defined for {@link java.lang.Object#equals(Object)} {@link
 * java.lang.Object#equals(Object)}
 *
 * <p>Implementing classes are stateful, because several methods require to set one record as the
 * reference for comparisons and later comparing a candidate against it. Therefore, the classes
 * implementing this interface are not thread safe. The runtime will ensure that no instance is used
 * twice in different threads, but will create a copy for that purpose. It is hence imperative that
 * the copies created by the {@link #duplicate()} method share no state with the instance from which
 * they were copied: they have to be deep copies.
 *
...
public abstract class TypeComparator<T> implements Serializable {
    ...
    /**
     * Computes a hash value for the given record. The hash value should include all fields in the
     * record relevant to the comparison.
     *
     * <p>The hash code is typically not used as it is in hash tables and for partitioning, but it
     * is further scrambled to make sure that a projection of the hash values to a lower cardinality
     * space is as results in a rather uniform value distribution. However, any collisions produced
     * by this method cannot be undone. While it is NOT important to create hash codes that cover
     * the full spectrum of bits in the integer, it IS important to avoid collisions when combining
     * two value as much as possible.
     *
    ...
    /**
     * Sets the given element as the comparison reference for future calls to {@link
     * #equalToReference(Object)} and {@link #compareToReference(TypeComparator)}. This method must
     ...
     *
     * <p>A typical example for checking the equality of two elements is the following:
     *
     * <pre>{@code
     * E e1 = ...;
     * E e2 = ...;
    ...
    /**
     * Compares two records in object form. The return value indicates the order of the two in the
     * same way as defined by {@link java.util.Comparator#compare(Object, Object)}.
     *
    ...
    /**
     * Compares two records in serialized form. The return value indicates the order of the two in
     * the same way as defined by {@link java.util.Comparator#compare(Object, Object)}.
     *
     * <p>This method may de-serialize the records or compare them directly based on their binary
     * representation.
     *
    ...
    /**
     * Gets the number of bytes that the normalized key would maximally take. A value of {@link
     ...
     * For example, if the data type is ordered with respect to an integer value it contains, then
     ...
     * specified position and writing exactly the given number of bytes. Note that the comparison of
     ...
     * org.apache.flink.types.NormalizableKey#copyNormalizedKey(MemorySegment, int, int)}. In the
     ...
     * serialized data. This must only be used when for all the key fields the full normalized key
     * is used. The method {@code #supportsSerializationWithKeyNormalization()} allows to check
     * that.
     *
    ...
    /**
     * Reads the record back while de-normalizing the key fields. This must only be used when for
     * all the key fields the full normalized key is used, which is hinted by the {@code
     * #supportsSerializationWithKeyNormalization()} method.
     *
    ...
    /**
     * Creates a copy of this class. The copy must be deep such that no state set in the copy
     * affects this instance of the comparator class.
     *
     ...
     * interoperability between different record types. Note, that at least one key should be
     * extracted.
     *
