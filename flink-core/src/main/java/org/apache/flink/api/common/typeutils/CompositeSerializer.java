 ...
 *
 * <p>This class serializes a composite type using array of its field serializers. Fields are
...
public abstract class CompositeSerializer<T> extends TypeSerializer<T> {
    ...
    protected CompositeSerializer(
            boolean immutableTargetType, TypeSerializer<?>... fieldSerializers) {
        this(
                PrecomputedParameters.precompute(
                        immutableTargetType, (TypeSerializer<Object>[]) fieldSerializers),
                fieldSerializers);
    }
    ...
    public CompositeSerializer<T> duplicate() {
        return precomputed.stateful
                ? createSerializerInstance(precomputed, duplicateFieldSerializers(fieldSerializers))
                : this;
    }

    private static TypeSerializer[] duplicateFieldSerializers(
    ...
    public T copy(T from, T reuse) {
        ...
        for (int index = 0; index < fieldSerializers.length; index++) {
            fields[index] =
                    fieldSerializers[index].copy(getField(from, index), getField(reuse, index));
        }
    ...
    public T deserialize(T reuse, DataInputView source) throws IOException {
        ...
        return precomputed.immutable
                ...
                : createInstanceWithReuse(fields, reuse);
    }
    ...
    protected static class PrecomputedParameters implements Serializable {
        ...
        private PrecomputedParameters(
                boolean immutableTargetType, boolean immutable, int length, boolean stateful) {
